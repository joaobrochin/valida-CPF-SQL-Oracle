...
SELECT TO_CHAR(DT_ABERTURA_SAC,'YYYY/MM')"ANO/MÊS DA ABERTURA DOS CHAMADOS",
COUNT( NR_SAC )"QUANTIDADE DE OCORRÊNCIAS"
FROM T_MC_SGV_SAC
GROUP BY DT_ABERTURA_SAC
ORDER BY DT_ABERTURA_SAC DESC;
...
SELECT  ROWNUM as RANK, 
        ...
        NR_TEMPO_TOTAL_SAC "TEMPO DE ATENDIMENTO em Horas"        
    FROM 
        ( SELECT    NR_SAC, 
                    NR_TEMPO_TOTAL_SAC 
        FROM T_MC_SGV_SAC 
...
FROM T_MC_DEPTO D INNER JOIN T_MC_FUNCIONARIO F
      ...
      INNER JOIN T_MC_SGV_SAC S
        ...
        WHERE F.ST_FUNC = 'A' and ST_SAC in ('F','X')
GROUP BY S.CD_FUNCIONARIO, F.NM_FUNCIONARIO, D.NM_DEPTO, F.DS_CARGO, F.ST_FUNC;
